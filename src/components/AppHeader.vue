...
async function showLogout() {
  let myModal = Modal.getOrCreateInstance(document.getElementById('#signout'));
  myModal.show();
}
...
<template>
  ...
  <!-- Sign in -->
  <AuthComponent :isLoggedIn="isLoggedIn" :currentUser="auth.currentUser" @close="hideAuth()"></AuthComponent>
